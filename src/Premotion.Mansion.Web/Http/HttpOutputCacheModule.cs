...
namespace Premotion.Mansion.Web.Http
{
	...
	/// </summary>
	public class HttpOutputCacheModule : DisposableBase, IHttpModule
	{
		...
		public class CachableWebResponse
		{
			...
			/// </summary>
			/// <param name="response">The <see cref="IHttpResponse"/> being cached.</param>
			/// <param name="contentBytes">The content of <paramref name="response"/>.</param>
			private CachableWebResponse(IHttpResponse response, byte[] contentBytes)
			{
				// validate arguments
				if (response == null)
			...
			public void Flush(IHttpResponse response)
			{
				// validate arguments
				if (response == null)
					throw new ArgumentNullException("response");
			...
			/// </summary>
			/// <param name="response">The <see cref="IHttpResponse"/> which is cached.</param>
			/// <param name="contentBytes">The content of <paramref name="response"/>.</param>
			/// <returns>Returns the created <see cref="CachedObject{TObject}"/>.</returns>
			public static CachedObject<CachableWebResponse> CreateCachedWebResponse(IHttpResponse response, byte[] contentBytes)
			{
				// validate arguments
				if (response == null)
					throw new ArgumentNullException("response");
		...
		private static void PostAcquireRequestState(object sender, EventArgs e)
		{
			...
			// get variables
			var httpContext = HttpContextAdapter.Adapt(HttpContext.Current);
			...
			var context = MansionHttpApplicationBase.ApplicationContext;
		...
		/// </summary>
		/// <param name="httpContext">The <see cref="IHttpContext"/> for which to generate the request.</param>
		/// <returns>Returns the generated <see cref="CacheKey"/>.</returns>
		public static CacheKey GenerateCacheKeyForRequest(IHttpContext httpContext)
		{
			// validate arugments
			if (httpContext == null)
			...
			// build de cache key
			return (StringCacheKey) string.Format("host={0}&port={1}&path={2}&get={3}&accept-encoding={4}&backoffice-user-authenticated={5}",
			                                      httpContext.Request["HTTP_HOST"],
			                                      httpContext.Request["SERVER_PORT"],
			                                      PathRewriterModule.GetOriginalRawPath(httpContext),
			                                      httpContext.Request["QUERY_STRING"],
			                                      httpContext.Request.GetAcceptEncodingHeader(),
		...
		/// </summary>
		/// <param name="response">The <see cref="IHttpResponse"/>.</param>
		/// <param name="cachedResponse">The <see cref="CachableWebResponse"/>.</param>
		public static void SetCacheControlProperties(IHttpResponse response, CachableWebResponse cachedResponse)
		{
			// validate arguments
			if (response == null)
				throw new ArgumentNullException("response");
