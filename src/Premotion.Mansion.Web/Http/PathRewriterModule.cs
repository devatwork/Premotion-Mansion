...
namespace Premotion.Mansion.Web.Http
{
	...
	/// </summary>
	public class PathRewriterModule : DisposableBase, IHttpModule
	{
		#region Constants
		/// <summary>
		...
		private static void BeginRequest(object sender, EventArgs e)
		{
			...
			if (StaticContentPrefix.Equals(pathParts[0], StringComparison.OrdinalIgnoreCase))
			{
			...
			else if (StreamingStaticContentPrefix.Equals(pathParts[0], StringComparison.OrdinalIgnoreCase))
			{
				// rewrite to dynamic resource
				httpContext.Items.Add(originalMappedPathKey, String.Join("/", pathParts, 1, pathParts.Length - 1));
			...
			else if (StaticResourcesPrefix.Equals(pathParts[0], StringComparison.OrdinalIgnoreCase))
			{
				// rewrite to dynamic resource
				httpContext.Items.Add(originalMappedPathKey, String.Join("/", pathParts, 1, pathParts.Length - 1));
			...
			else if (DynamicResourcesPrefix.Equals(pathParts[0], StringComparison.OrdinalIgnoreCase))
			{
				// rewrite to dynamic resource
				httpContext.Items.Add(originalMappedPathKey, String.Join("/", pathParts, 1, pathParts.Length - 1));
			...
			else if (MergeResourcesPrefix.Equals(pathParts[0], StringComparison.OrdinalIgnoreCase))
			{
		...
		/// </summary>
		/// <param name="context">The <see cref="IHttpContext"/> of the current request.</param>
		/// <returns>Returns the orginal <see cref="Uri"/>.</returns>
		public static Uri GetOriginalRawUrl(IHttpContext context)
		{
			// validate arguments
			if (context == null)
				throw new ArgumentNullException("context");
		...
		public static string GetOriginalRawPath(IHttpContext context)
		{
			// validate arguments
			if (context == null)
				throw new ArgumentNullException("context");
		...
		public static string GetOriginalMappedPath(IHttpContext context)
		{
			// validate arguments
			if (context == null)
				throw new ArgumentNullException("context");
