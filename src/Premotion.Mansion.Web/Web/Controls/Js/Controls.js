...
Author: Premotion
========================================================================== */
// TODO:  http://unwrongest.com/projects/lazy/
...
function resizeFormElements($)
{
	$(this).ready(function() {
		...
		$("form.inline-form:not(.no-resize)").each(function () {
			// reset width
			$(this).find("label:not(.field label)").each(function () {
				$(this).width("auto");
			});
			...
			// loop through all the labels to determine the widest one
			var maxLabelWidth = 0;
			$(this).find("label:not(.field label)").each(function () {
			...
			$(this).find(".field-prefix").each(function () {
				width = $(this).outerWidth(true);
				if (width > maxPrefixLength) {
					maxPrefixLength = width;
				}
			});
			var maxPostfixLength = 0;
			$(this).find(".field-postfix").each(function () {
				width = $(this).outerWidth(true);
				if (width > maxPostfixLength) {
					maxPostfixLength = width;
				}
			});

			// loop through all its fieldsets
			$(this).find("fieldset").each(function () {

				// set the width of the labels and field
				$(this).find("label:not(.field label)").each(function () {
					$(this).width(maxLabelWidth);
					var field = $(this).siblings(".field");
					if (field != null) {
						var width = $(this).parent().width() - (maxLabelWidth + 12);
						if (field.hasClass("small-field"))
							width = $(this).parent().width() / 3;
						if (field.hasClass("selectbox"))
							width = width + 6;

						// determine the left margin
						var fieldPrefixWidth = 0;
						var fieldPrefix = $(this).siblings(".field-prefix");
						if ( fieldPrefixWidth != null )
							fieldPrefixWidth = fieldPrefix.outerWidth(true);
						if (fieldPrefixWidth < maxPrefixLength)
							field.css( "margin-left", (maxPrefixLength - fieldPrefixWidth) + "px" );

						// set the width
						field.width(width - maxPostfixLength - maxPrefixLength);
					}
				});
			});
		});
...
;(function ($, undefined) {
	$(this).ready(function() {
		...
		/* grid filters */
		$(".filter-textbox").live('keypress', function(e) {
			if ((e.which && e.which === 13) || (e.keyCode && e.keyCode === 13)) {
				var controlId = $(this).parents(".grid").attr("id");
				// reset current page
				$("select[name=" + controlId + "_page-number]").val(1);
				// submit using update ui button
				$(this).parents('form').find(".updateui").click();
				return false;
			}
			return true;
		});
		...
		/* paging controls */
		$(".paging-control").each(function() {
			var pagingControlId = $(this).attr("id");
			var pageSizeControl = $(this).find("#" + pagingControlId + "-page-size");
			var pageNumberControl = $(this).find("#" + pagingControlId + "-page-number");
			var pageTotalControl = $(this).find("#" + pagingControlId + "-page-total");
			var updateUiButton = $(this).parents('form').find(".updateui");

			// reset page number when changing page size
			pageSizeControl.change(function() {
				pageNumberControl.val(1);
				updateUiButton.click();
				return false;
			});

			// update ui when page number has changed
			pageNumberControl.change(function() {
				updateUiButton.click();
				return false;
			});

			// attach paging button handlers
			$(this).find("a.first-page").click(function() { pageNumberControl.val(1); updateUiButton.click(); return false; });
			$(this).find("a.previous-page").click(function() { pageNumberControl.val( parseInt(pageNumberControl.val()) - 1 ); updateUiButton.click(); return false; });
			$(this).find("a.next-page").click(function() { pageNumberControl.val( parseInt(pageNumberControl.val()) + 1 ); updateUiButton.click(); return false; });
			$(this).find("a.last-page").click(function() { pageNumberControl.val( pageTotalControl.text() ); updateUiButton.click(); return false; });
		});
	});
})(window.jQuery);