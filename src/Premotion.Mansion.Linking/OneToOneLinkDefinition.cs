...
{
	/// <summary>
	/// Implements a one-to-one relation. Which allows exactly one <see cref="Link"/> of this type in the <see cref="Linkbase"/>.
	/// </summary>
	public class OneToOneLinkDefinition : LinkDefinition
	{
		#region Constructors
		/// <summary>
		...
		public OneToOneLinkDefinition(string name) : base(name)
		{
		}
		#endregion
		...
		protected override Link CreateToLink(IMansionContext context, Linkbase source, Linkbase target, IEnumerable<KeyValuePair<string, object>> properties)
		{
			...
			if (target.Links.OfType(this).Any())
