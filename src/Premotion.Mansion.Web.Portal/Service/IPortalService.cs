...
namespace Premotion.Mansion.Web.Portal.Service
{
	...
	public interface IPortalService : IService
	{
		...
		bool TryResolveTemplatePage(MansionContext context, Node siteNode, Node contentNode, out Node templatePageNode);
		#endregion
		...
		void RenderColumn(MansionContext context, string columnName, IPropertyBag ownerProperties, Dataset blockDataset, string targetField);
		/// <summary>
		...
		Dataset GetColumnDataset(MansionContext context, ITypeDefinition type);
		#endregion
		...
		/// <param name="targetField">The name of the field to which to render.</param>
		void RenderBlock(MansionContext context, IPropertyBag blockProperties, string targetField);
		#endregion
	}
}