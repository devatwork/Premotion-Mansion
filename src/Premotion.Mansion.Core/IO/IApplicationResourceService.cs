...
namespace Premotion.Mansion.Core.IO
{
	...
	public interface IApplicationResourceService : IResourceService
	{
		...
		IResource GetSingle(IContext context, IResourcePath path);
		/// <summary>
		...
		/// <exception cref="ResourceNotFoundException">Thrown when a resource can not be resolved from it's path.</exception>
		IEnumerable<IResource> Get(IContext context, IResourcePath path);
		/// <summary>
		...
		bool TryGet(IContext context, IResourcePath path, out IEnumerable<IResource> resources);
		#endregion
		...
		IEnumerable<string> EnumeratorFolders(IContext context, string path);
		#endregion
	}
}