...
namespace Premotion.Mansion.Core.Caching
{
	...
	/// </summary>
	public interface ICachingService : IService
	{
		...
		bool TryGet<TObject>(IContext context, CacheKey key, out CachedObject<TObject> value);
		/// <summary>
		...
		TObject GetOrAdd<TObject>(IContext context, CacheKey key, Func<CachedObject<TObject>> valueFactory);
		/// <summary>
		...
		/// <returns>Returns the value.</returns>
		TObject AddOrReplace<TObject>(IContext context, CacheKey key, Func<CachedObject<TObject>> valueFactory);
		/// <summary>
