	...
	public class StatusClauseConverter : ClauseConverter<StatusClause>
	{
		...
		protected override void Map(IMansionContext context, Schema schema, SqlCommand command, SqlStringBuilder queryBuilder, StatusClause clause)
		{
			...
			if ((clause.Status & NodeStatus.Staged) == NodeStatus.Staged)
				buffer.AppendFormat("{1}([{0}].[approved] = 1 AND [{0}].[publicationDate] > GETDATE() AND [{0}].[archived] = 0)", schema.RootTable.Name, buffer.Length > 2 ? " OR " : string.Empty);

			if ((clause.Status & NodeStatus.Published) == NodeStatus.Published)
				buffer.AppendFormat("{1}([{0}].[approved] = 1 AND [{0}].[publicationDate] <= GETDATE() AND [{0}].[expirationDate] >= GETDATE() AND [{0}].[archived] = 0)", schema.RootTable.Name, buffer.Length > 2 ? " OR " : string.Empty);

			if ((clause.Status & NodeStatus.Expired) == NodeStatus.Expired)
				buffer.AppendFormat("{1}([{0}].[approved] = 1 AND [{0}].[expirationDate] < GETDATE() AND [{0}].[archived] = 0)", schema.RootTable.Name, buffer.Length > 2 ? " OR " : string.Empty);
