...
{
	/// <summary>
	/// Returns a comma separated list of synonyms.
	/// </summary>
	[ScriptFunction("GetSynonymList")]
	public class GetSynonymList : FunctionExpression
	{
		#region Constructors
		/// <summary></summary>
		/// <param name="resourceService"></param>
		/// <exception cref="ArgumentNullException"></exception>
		public GetSynonymList(IApplicationResourceService resourceService)
		{
			// validate arguments
		...
		public string Evaluate(IMansionContext context, string language)
		{
			...
			using (var pipe = resource.Last().OpenForReading())
			{
				...
				while ((line = pipe.Reader.ReadLine()) != null)
				{
					...
					synonyms.Add(string.Join(", ", pairs));
				}
			}

			// format
			return string.Join(Environment.NewLine, synonyms);
		}
		#endregion
